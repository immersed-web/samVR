<template>
  ...
  <div class="w-full aspect-video relative">
    <div class="rounded-br-xl bg-neutral/60 absolute p-2 top-0 left-0 z-10">
      <form v-if="isEditingCameraName" class="flex flex-nowrap items-center gap-2" @submit.prevent="setCameraName">
        <input v-model="camera.currentCamera!.name" type="text" class="input input-sm">
      ...
      <div v-else class="flex flex-nowrap items-center gap-2">
        ...
        <button @click="isEditingCameraName = true" class="btn btn-sm btn-primary btn-circle">
      ...
      <div class="form-control">
        <label class="label">
          <input type="checkbox"
            ...
            :checked="camera.is360Camera" class="toggle toggle-primary">
        ...
        <label class="label">
          <input type="checkbox"
            ...
            :checked="camera.isRoofMounted" class="toggle toggle-primary">
    ...
    <div class="bottom-0 absolute w-full bg-neutral/50 flex flex-row gap-4 justify-center p-4">
      <template v-for="listedCamera in camerasWithPortalInfo" :key="listedCamera.cameraId">
        <div v-if="listedCamera.cameraId !== camera.currentCamera?.cameraId">
          <div v-if="listedCamera.hasPortal"
            ...
            <button @click="CameraViewRef?.createOrCenterOnPortal(listedCamera.cameraId)"
            ...
            <button @click="adminStore.deletePortal(camera.currentCamera!.cameraId, listedCamera.cameraId)"
