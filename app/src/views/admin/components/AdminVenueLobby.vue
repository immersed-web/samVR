<template>
  <div>
    <div class="flex mb-4" v-auto-animate>
      <h2 class="flex-1">
        VR-lobby
      </h2>
      <button v-if="!venueStore.currentStream?.vrSpace" class="btn btn-primary" @click="createVirtualSpace">
        Lägg till VR-lobby
      </button>
      <!-- <button
        v-else
    ...
    <div v-auto-animate>
      <div v-if="venueStore.currentStream?.vrSpace" class="flex flex-col gap-4">
        ...
        <div v-if="venueStore.modelUrl" class="grid gap-2">
          <VrAFramePreview class="flex-1 border" :model-url="venueStore.modelUrl" :navmesh-url="venueStore.navmeshUrl"
        ...
        <div>
          ...
          <div v-if="venueStore.currentStream?.vrSpace?.virtualSpace3DModel">
        ...
        <div class="flex gap-4">
          ...
          <input class="rounded-md border-black border border-2" type="color"
            :value="venueStore.currentStream.vrSpace.virtualSpace3DModel.skyColor" @input="setSkyColor">
          <!-- <p>{{ skyColor }}</p> -->
        </div>
      </div>
    </div>
...
onMounted(() => {
  ...
  const sRadius = venueStore.currentStream?.vrSpace?.virtualSpace3DModel?.spawnRadius;
  if (sRadius) {
...
watch(entranceRotation, (rot) => {
  if (!venueStore.currentStream?.vrSpace?.virtualSpace3DModel) return;
  venueStore.currentStream.vrSpace.virtualSpace3DModel.entranceRotation = rot;
});

const spawnRadius = ref(0);
watch(spawnRadius, (radius) => {
  if (!venueStore.currentStream?.vrSpace?.virtualSpace3DModel) return;
  venueStore.currentStream.vrSpace.virtualSpace3DModel.spawnRadius = radius;
});
