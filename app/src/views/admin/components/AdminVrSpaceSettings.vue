<template>
  <div>
    <div class="flex mb-4" v-auto-animate>
      ...
      <button v-if="!streamStore.currentStream?.vrSpace" class="btn btn-primary" @click="createVirtualSpace">
        Lägg till VR-lobby
      </button>
    </div>
    <div v-auto-animate>
      <div v-if="streamStore.currentStream?.vrSpace" class="flex flex-col gap-4">
        ...
        <div v-if="streamStore.modelUrl" class="grid gap-2">
          <VrAFramePreview class="flex-1 border" :model-url="streamStore.modelUrl" :navmesh-url="streamStore.navmeshUrl"
...
onMounted(() => {
  const storeRot = streamStore.currentStream?.vrSpace?.virtualSpace3DModel?.entranceRotation;
  if (storeRot) {
    entranceRotation.value = storeRot;
  }
  const sRadius = streamStore.currentStream?.vrSpace?.virtualSpace3DModel?.spawnRadius;
  if (sRadius) {
...
watch(entranceRotation, (rot) => {
  if (!streamStore.currentStream?.vrSpace?.virtualSpace3DModel) return;
  streamStore.currentStream.vrSpace.virtualSpace3DModel.entranceRotation = rot;
});

const spawnRadius = ref(0);
watch(spawnRadius, (radius) => {
  if (!streamStore.currentStream?.vrSpace?.virtualSpace3DModel) return;
  streamStore.currentStream.vrSpace.virtualSpace3DModel.spawnRadius = radius;
});
