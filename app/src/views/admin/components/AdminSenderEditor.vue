<template>
  <div class="w-full aspect-video bg-base-200">
    <video class="" ref="videoTag" id="main-video" autoplay />

    <div class="flex flex-row gap-2 justify-center p-4">
      <button v-if="newName === undefined" class="btn btn-primary btn-sm" @click="newName = ''">
        skapa ny kameraprofil
      </button>
      <form v-else class="card p-2 bg-slate-400 flex flex-col flex-nowrap gap-2"
        ...
        <div class="flex flex-nowrap gap-2">
          <button type="submit" class="btn btn-primary flex-auto">
            <span class="material-icons">add</span>
          </button>
          <button @click="newName = undefined" class="btn flex-auto">
            <span class="material-icons">cancel</span>
          </button>
        </div>
      </form>
      <template v-for="listedCamera in adminStore.adminOnlyStreamState?.cameras" :key="listedCamera.cameraId">
        <button v-if="!listedCamera.senderAttached" class="btn btn-sm"
...
const props = defineProps<{
  sender: NonNullable<typeof adminStore.adminOnlyStreamState>['detachedSenders'][ConnectionId]
}>();
