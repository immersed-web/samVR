...
const crumbs = computed(() => {
  const filtered = route.matched.filter(m => !(m.meta.breadcrumbIgnore || m.path == '/'));
  return filtered.map(m => { return { path: m.path, crumb: m.meta.breadcrumb || m.name || m.path || '?' }; });
});
...
<template>
  <div class="breadcrumbs text-sm">
    <ul>
      <!-- <div class="btn btn-xs btn-circle btn-warning" @click="console.log('Breadcrumbs', crumbs)">
      ...
      <template v-for="(c, i) in crumbs" :key="i">
        <li>
          <component :is="i < crumbs.length - 1 ? 'router-link' : 'span'" :to="{ path: c.path }">
            {{ c.crumb }}
          </component>
        </li>
      </template>
