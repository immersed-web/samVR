<template>
  <div class="text-white navbar bg-primary">
    <div class="flex-1">
      <RouterLink :to="{name: authStore.routePrefix + 'Home'}" class="text-xl normal-case btn btn-ghost">
        SamVR
      </RouterLink>
    </div>
    <div class="flex-none">
      <div v-if="authStore.isNotGuest" class="text-black dropdown dropdown-end">
        ...
        <ul tabindex="0" class="p-2 mt-3 shadow menu menu-compact dropdown-content bg-base-100 rounded-box w-52">
          ...
          <li>
            <a class="hover:bg-error" @click="logout">Logga ut</a>
          </li>
        </ul>
      </div>
      <slot />
